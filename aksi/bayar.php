...
if ($_POST) {
    if ($_POST['aksi'] == 'tambah') {
        ...
        while ($tagihan = mysqli_fetch_array($query_tagihan)) {
            ...
            // insert tabel bayar alokasi
            $sql_alokasi = "INSERT bayar_alokasi(id_bayar_alokasi,id_bayar,id_tagihan,total_alokasi,dialokasikan_oleh,dibuat_pada,diubah_pada,dihapus_pada) VALUES(DEFAULT,$id_bayar,$id_tagihan,$dibayarkan,'otomatis oleh sistem',DEFAULT,DEFAULT,DEFAULT)";
            ...
            // echo "<br>Dibayarkan untuk".$tagihan['deskripsi_biaya']." Besar ".number_format($dibayarkan);
            if ($alokasi_dana <= 0) {
                break;
            }
        }
        // ubah status verifikasi pembayran ( jika yang membayar adalah petugas maka status sudah verikasi namun jika yang membayar adalah siswa/ortu siswa maka status belum verifikasi)
    ...
    } else if ($_POST['aksi'] == 'tambah-siswa') {
        $id_siswa = $_POST['id_siswa'];
        $id_bayar_metode = $_POST['id_bayar_metode'];
        $keterangan = $_POST['keterangan'];
        $status_verifikasi = "belum verifikasi";
