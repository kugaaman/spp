...
session_start();
    ...
    if($_POST){
        if ($_POST['aksi'] =='tambah'){
            ...
            nama='$nama', tingkat=$tingkat,kelas='$kelas',
            id_jurusan=$id_jurusan, alamat='$alamat',
...
if($_GET){
    if($_GET['aksi']== 'hapus'){
        ...
        // HARD DELETE 
        $sql="UPDATE siswa SET dihapus_pada=now() WHERE
        id_siswa=$id_siswa"; // Soft Delete 
        ...
        $id_siswa=$_GET['id_siswa'];
        $sql="UPDATE siswa SET dihapus_pada= NULL WHERE
        id_siswa=$id_siswa";
        mysqli_query($koneksi,$sql);
        notifikasi($koneksi);
        header('location:../index.php?p=siswa');
